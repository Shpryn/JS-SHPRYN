...
function createFood() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        } while (bomb_x == food_x & bomb_y == food_y)

        var food_cell = document.getElementsByClassName('cell-' + food_y + '-' + food_x)[0];
        var food_cell_classes = food_cell.getAttribute('class').split(' ');

        var bombDel = document.querySelector(".bomb-unit");
        var bomb_cell = document.getElementsByClassName('cell-' + bomb_y + '-' + bomb_x)[0];
 
        if(bombDel){
            bombDel.classList.remove("bomb-unit");
        }
...
function teleport(crd_y, crd_x) {
    var tp;
    if (direction == 'x-') {
        tp = document.getElementsByClassName('cell-' + (crd_y) + '-' + (FIELD_SIZE_X-1  ))[0];
    }
    else if (direction == 'x+') {
        tp = document.getElementsByClassName('cell-' + (crd_y) + '-' + (0))[0];
    }
    else if (direction == 'y+') {
        tp = document.getElementsByClassName('cell-' + (FIELD_SIZE_Y-1) + '-' + (crd_x))[0];
    }
    else if (direction == 'y-') {
        tp = document.getElementsByClassName('cell-' + (0) + '-' + (crd_x))[0];
    }
    return tp;
}
/**
